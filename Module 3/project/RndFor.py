...
for n_est in (1, 4, 20):
    ...
    rfc = RandomForestClassifier(max_depth=None, n_estimators=n_est, criterion='entropy', random_state=42)

    # Fit the model
    rfc.fit(x_train_transformed, y_train)

    # Print the training scores
    print("Training score : %.3f (n_est=%d)" % (rfc.score(x_train_transformed, y_train), n_est))
    ...
    ax[0].set_title("Decision surface of RFC (n_est=%d)" % n_est)

    # Plot the decision tree
    plot_tree(rfc.estimators_[0], ax=ax[1], filled=True, feature_names=["continuous_time"])
    plt.tight_layout()
    plt.show()
    ...
    rfc_mse = mean_squared_error(y_test, flattened_predictions)
    ...
    print(f'Random Forest Mean Squared Error: {rfc_mse}')  # Fix the variable name here
